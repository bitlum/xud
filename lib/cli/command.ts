...
export const callback = (argv: Arguments, formatOutput?: Function, displayJson?: Function) => {
  return (error: grpc.ServiceError | null, response: GrpcResponse) => {
    if (error) {
      ...
      } else if (error.code === grpc.status.UNIMPLEMENTED && error.message.includes('xud node cannot be created because it already exists')) {
        console.error("an xud node already exists, try unlocking it with 'xucli unlock'");
      } else if (error.code === grpc.status.UNIMPLEMENTED && error.message.includes('xud node cannot be unlocked because it does not exist')) {
